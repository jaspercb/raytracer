        ...
        for j in 0..2 {
            ...
            for k in 0..2 {
                ...
                accum += (fi*uu + (1.0-fi)*(1.0-uu)) *
                         (fj*vv + (1.0-fj)*(1.0-vv)) *
                         (fk*ww + (1.0-fk)*(1.0-ww)) * weight_v.dot(c[i][j][k]);
            }
        }
    }
    return accum;
...
impl Perlin {
    pub fn new() -> Perlin {
        ...
        return Self {ranvec, perm_x, perm_y, perm_z}
    }
    ...
    fn generate() -> Vec<Vec3> {
        ...
        let mut ret = Vec::new();
        for _ in 0..256 {
            ret.push(Vec3::new(-1.0 + 2.0 * rng.gen::<f64>(), -1.0 + 2.0 * rng.gen::<f64>(), -1.0 + 2.0 * rng.gen::<f64>()));
        }
        return ret;
    }
    ...
    pub fn noise(&self, p: &Vec3) -> f64 {
        ...
        for di in 0..2 {
            for dj in 0..2 {
                for dk in 0..2 {
                    c[di][dj][dk] = self.ranvec[self.perm_x[(i + di as f64) as usize & 255] ^ self.perm_y[(j + dj as f64) as usize & 255] ^ self.perm_z[(k + dk as f64) as usize & 255]];
                }
            }
        }

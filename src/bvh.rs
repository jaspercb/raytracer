...
impl BvhNode {
    pub fn construct(hittables: &mut Vec<Box<dyn Hittable>>, t0: f64, t1: f64) -> Box<dyn Hittable> {
...
impl Hittable for BvhNode {
    fn hit(&self, r: &Ray, t_min: f64, t_max: f64) -> Option<HitRecord> {
        if self.bound.hit(r, t_min, t_max) {
            return match (self.left.hit(r, t_min, t_max), self.right.hit(r, t_min, t_max)) {
                (None, right) => right,
                (left, None) => left,
                (Some(a), Some(b)) => {
